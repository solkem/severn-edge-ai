...
export function TestPage({
  ...
  const appendInferenceFrame = useCallback((
    prediction: number,
    confidenceValue: number,
    timestampMs?: number,
  ) => {
    const frameTimestamp = Number.isFinite(timestampMs) ? (timestampMs as number) : Date.now();
  ...
  const registerPacketTiming = useCallback((packetTimestamp: number): number => {
    ...
    if (
      ...
      && packetTimestamp < telemetry.lastPacketTimestamp16 - 32768
    ) {
      telemetry.packetTimestampWrapOffsetMs += 65536;
    }
    telemetry.lastPacketTimestamp16 = packetTimestamp;

    const packetTimestampExtendedMs =
      telemetry.packetTimestampWrapOffsetMs + packetTimestamp;

    if (telemetry.clockOffsetMs === null) {
      telemetry.clockOffsetMs = now - packetTimestampExtendedMs;
    }

    if (telemetry.lastPacketTimestampExtendedMs !== null) {
      const dtMs = packetTimestampExtendedMs - telemetry.lastPacketTimestampExtendedMs;
      if (dtMs > 0 && dtMs < 500) {
        telemetry.sampleDtEmaMs = emaUpdate(telemetry.sampleDtEmaMs, dtMs);
      }
    }
    ...
    try {
      if (useArduinoInference) {
        await ble.startInference((result: InferenceResult) => {
          ...
          const modelWindowMs = MODEL_CONFIG.WINDOW_SIZE * DEFAULT_SAMPLE_DT_MS;
        ...
        await ble.startSensorStream((packet) => {
          const latestPacketLocalTimestampMs = registerPacketTiming(packet.timestamp);
          sampleBuffer.push([
